...
const noop = () => {};
let mangoIndexesExtension = noop;
let publishExtension = noop;

if (Meteor.isServer) {
    mangoIndexesExtension = require('./UniMongoIndexes').default;
    publishExtension = require('./UniPublish').default;
}

export const _uniExtensions = {
    schemaExtension, mangoIndexesExtension, methodsExtension, aggregateExtension, hooksExtension, publishExtension
};
...
export function createUniCollection ({CollectionClass = Mongo.Collection, _extensions = _.values(_uniExtensions)} = {}) {
