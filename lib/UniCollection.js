...
UniCollection = function () {
    ...
    options.documentPrototype = options.documentPrototype || options.docConstructor;
    if (options.documentPrototype) {
        if (!_.isFunction(options.documentPrototype)) {
            throw new Error('documentPrototype must be a function.');
        }
        constructor = options.documentPrototype;
    } else {
...
UniCollection.prototype.setDocumentPrototype = function (docConstructor, direct) {
    ...
    self._uniTrigger('setDocumentPrototype', this._docConstructor.prototype);
    _.each(self._docHelpers, function (helper, name) {
        self._docConstructor.prototype[name] = helper;
    });
    this._transform = function (doc) {
        var docInstance = new self._docConstructor(doc);
        self._uniTrigger('transformDocument', docInstance);
        return docInstance;
    };
};
...
UniCollection.prototype._initMixins = function (options) {
...
UniCollection.prototype._uniTrigger = function (eventName, args) {
    var self = this;
    if (_.isArray(this._mixinEvents[eventName])) {
...
UniCollection.prototype.getFieldOptionsFromSchema = function (fieldName) {
    if (!_.isFunction(this.getSimpleSchema)) {
        throw new Meteor.Error('Simple Schema is not attached on this collection!');
    }
    if (_.isString(fieldName)) {
        var field = this.getSchema().schema(fieldName);
        if (!field) {
...
UniCollection.prototype._registerDocConstructorAsANewEJSONType = function () {
    ...
    EJSON.addType(self._name + 'Doc', function (value) {
        return self.create(EJSON.fromJSONValue(value));
    });
};
 ...
 */
UniCollection.prototype.create = function (rawDoc, options) {
    rawDoc = rawDoc || {};
    var doc = this._transform(rawDoc);
    if (options && (_.isBoolean(options) || options.save)) {
        doc.save(_.omit(options, 'save'));
    }
    return doc;
};
...
if (Meteor.isServer) {
     ...
     * @deprecated Please don't use. It will be removed or moved to separated package.
     */
