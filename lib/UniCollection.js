...
UniCollection = class UniCollection extends Meteor.Collection {
    ...
    _init (name, options = {}) {
        ...
        let docCls;
        options.mixins && this._initMixins(options.mixins, options);
    ...
    _initMixins(mixins, options) {
        if (!Array.isArray(mixins)){
            throw new Error('Parameters mixins should be an array!');
        }
        let unnamed = 0;
        mixins.forEach((mixin) => {
            if(typeof mixin === 'object' && mixin instanceof UniCollection.UniCollectionAbstractMixin){
                mixin.init(this, options);
                this._mixinInstances[mixin.name || unnamed++] = mixin;
            }
