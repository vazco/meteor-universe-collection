...
UniCollection = function () {
    ...
    if(_.isArray(options.mixins)){
        this._mixinInstances = _.map(options.mixins, function(mixin){
            return new mixin(self, options);
        });
    }
...
UniCollection.prototype.setDocumentPrototype = function (docConstructor, direct) {
    ...
    self._trigger('setDocumentPrototype', this._docConstructor.prototype);
    ...
    this._transform = function (doc) {
        ...
        self._trigger('transformDocument', docInstance);
        return docInstance;
    };
};
...
UniCollection.prototype._trigger = function(eventName, args){
    eventName = 'on'+UniUtils.capitalizeFirst(eventName);
    var self = this;
    var fn = function(mixin){
        _.isFunction(mixin[eventName]) && mixin[eventName](args, self);
    };
    this._mixinInstances && this._mixinInstances.forEach(fn);
};
