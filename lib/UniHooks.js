 ...
 * });
    ...
    const getDocsForHooks = function getDocForHooks (ctx, [selector, modifierOrOptions], fnName) {
        const areAfterHooks = !!countAfterHooksToCall.call(ctx.getCollection(), fnName);
        const cache = areAfterHooks && ctx.getCollection().find(selector, {transform: null}).fetch();
        let insts;
        ctx.getPreviousDocs = plainObject => {
            const docs = areAfterHooks? cache : (cache || ctx.getCollection().find(selector, {transform: null}).fetch());
            return plainObject?
                docs
                :
                insts? insts : insts = docs.map(d => ctx.getCollection().create(d));
        };
    ...
    const wrapWithHooks = function wrapWithHooks (uniCol, fnName, methodForHooking, canBeAsync, attachSpecificHelpers) {
        return function (...args) {
            if (hooksVars.get() && hooksVars.get().includes('ALL')) {
    ...
    UniCollection.prototype._init = function () {
        Object.keys(LIST_ALL).forEach(fnName => {
            ...
            UniUtils.set(this, pathToFn, wrapWithHooks(
                ...
                !Object.values(LIST_WITH_NO_CALLBACK).includes(fnName),
...
// wrapping remote methods (on documents) with hook mechanism
    ...
    function runBeforeHooks (methodName, args, context, isAsync) {
        ...
        if (ommit.includes('BEFORE')) {
            return true;
        }
        var hooks = UniUtils.get(this, '_hooks.before.' + normalizeMethodName(methodName)) || {};
        context.isAfter = () => false;
        return Object.keys(hooks).every(idName => {
            if (ommit.includes(idName)) {
                return true;
            }
    ...
    function countAfterHooksToCall (methodName) {
        const ommit = hooksVars.get() || [];
        if (ommit.includes('AFTER')) {
            return 0;
        }
        return Object.keys(UniUtils.get(this, '_hooks.after.' + normalizeMethodName(methodName)) || {}).length;
    }
    ...
    function runAfterHooks (methodName, args, context) {
        ...
        var hooks = UniUtils.get(this, '_hooks.after.' + normalizeMethodName(methodName)) || {};
