...
export class UniDoc {
    ...
    save (options, cb) {
        ...
        if (!Array.isArray(fieldsList)) {
            ...
            fieldsList = schema ? schema.objectKeys() : Object.keys(this);
        }
        var obj = Object.keys(this).filter(key => fieldsList.includes(key))
            .reduce((newObj, key) => {
                newObj[key] = this[key];
                return newObj;
            }, {});
        ...
        const objWithoutId = Object.keys(obj).filter(key => key !== '_id')
            .reduce((newObj, key) => {
                newObj[key] = obj[key];
                return newObj;
            }, {});
        if (Meteor.isServer && !isFunction(cb)) {
            return !!this.update({$set: objWithoutId}, {useSchema: options.useSchema});
        }
        return !!this.update({$set: objWithoutId}, {useSchema: options.useSchema}, function (err) {
            if (err && !cb) {
                Meteor._debug(err);
                return;
            }
