...
UniDoc.prototype.save = function (options, cb) {
    ...
    if (!this._id) {
        if (Meteor.isServer && !_.isFunction(cb)) {
            this._id = this.getCollection().insert(obj, {useSchema: options.useSchema});
            return this._id;
        }
        return this.getCollection().insert(obj, {useSchema: options.useSchema}, function (err, res) {
            if (err && !cb) {
                Meteor._debug(err);
                return;
            }
    ...
    if (Meteor.isServer && !_.isFunction(cb)) {
        return !!this.update({$set: obj}, {useSchema: options.useSchema});
    }
