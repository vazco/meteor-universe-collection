...
var UniDoc = class UniDoc {
     ...
     * @returns {*}
     */
    update (modifier, options = {}, cb = null) {
    ...
    save (options, cb) {
        ...
        if (Meteor.isServer && !_.isFunction(cb)) {
            console.log('this->d', this._id, this.getCollectionName());
            var res = !!this.update({$set: _.omit(obj, '_id')}, {useSchema: options.useSchema});
            console.log('r', res);
            return res;
        }
    ...
    apply () {
        ...
        return coll.apply.apply(this, _.union(arr, args));
    }
