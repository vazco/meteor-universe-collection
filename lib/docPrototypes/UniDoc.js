...
var UniDoc = class UniDoc {
     ...
     * @returns {*}
     */
    update (modifier, options = {}, cb:Function = null) {
    ...
    save (options, cb) {
        ...
        if (!this._id || !this.getCollection().find(this._id).count()) {
            var col = (options.direct && this.getCollection().direct) || this.getCollection();
            if (Meteor.isServer && !_.isFunction(cb)) {
    ...
    getSchema () {
        var col = this.getCollection();
        return col.getSchema.apply(col, arguments);
    }
