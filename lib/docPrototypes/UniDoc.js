 ...
 * @param {String|[String]} fieldsList name or array with names of fields to save
 * @param {Function} cb callback
 * @returns {*} status of operation or id of new doc
 */
UniDoc.prototype.save = function (fieldsList, cb) {
    var self = this;
    if (!_.isFunction(cb) && _.isFunction(fieldsList)) {
        cb = fieldsList;
    ...
    if (!_.isArray(fieldsList)) {
        fieldsList = this._keys;
    }
    var obj = _.pick(this, fieldsList);
    if (!this._id) {
        return this.insert(obj, function (err, res) {
            if (err && !cb) {
                Meteor._debug(err);
                return;
            }
    ...
    return !!this.update({$set: obj}, function (err) {
        if (err && !cb) {
            Meteor._debug(err);
            return;
        }
