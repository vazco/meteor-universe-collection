 ...
 */
UniCollection.prototype.setSchema = function uniAttachSchema (name, ss, options) {
    var self = this;
    options = options || {};
...
_.each(['insert', 'update', 'upsert'], function (methodName) {
    ...
    UniCollection.prototype[methodName] = function () {
        ...
        if (Meteor.isServer) {
            if (methodName === 'insert' && args.length > 2 && typeof args[2] !== 'function') {
                args.splice(2, 1);
            }
            if (_.contains(['update', 'upsert'], methodName) && args.length >= 3 && !args[2]) {
                args.splice(2, 1);
            }
        }
...
var _remove = UniCollection.prototype.remove;
UniCollection.prototype.remove = function () {
    var args = _.toArray(arguments);
    if (Meteor.isServer && args.length > 1 && typeof args[1] !== 'function') {
        args.splice(1, 1);
    }
...
function defineDeny (c, options) {
    var _validatedInsert, _validatedUpdate, isLocalCollection;
    if (Meteor.isServer && !alreadyDefined[c.getCollectionName()]) {
        c._tmpInsertOptions = new Meteor.EnvironmentVariable();
        _validatedInsert = c._validatedInsert;
        c._validatedInsert = function () {
            var self = this, args = arguments, generatedId, options;
            ...
            return this._tmpInsertOptions.withValue(options, function () {
                // backing to proper arguments: userId, doc, generatedId
                return _validatedInsert.call(self, args[0], args[1], generatedId);
            });
        };

        _validatedUpdate = c._validatedUpdate;
        c._tmpUpdateOptions = new Meteor.EnvironmentVariable();
        c._validatedUpdate = function (userId, selector, mutator, options) {
            var self = this;
            var args = arguments;
            return this._tmpUpdateOptions.withValue(options, function () {
                return _validatedUpdate.apply(self, args);
            });
        };
        ...
        c.deny({
            insert: function (userId, doc) {
                var options = c._tmpInsertOptions.get() || {};
            ...
            update: function (userId, doc, fields, modifier) {
                var options = c._tmpUpdateOptions.get() || {};
                var ss = c.getSchema(options.useSchema);
